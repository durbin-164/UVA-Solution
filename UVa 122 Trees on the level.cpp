...
bool cmp(PS a, PS b)
{
    if(siz(a.ff)==siz(b.ff))return a.ff<b.ff;

    return siz(a.ff)<siz(b.ff);
}

vector<PS>v;

bool makenode(string s)
{
    stringstream ss(s);
    string s1,s2,s3;
...
bool valid()
{
    ...
    for(int i=1; i<siz(v); i++)
    {
        ...
        if(mp[s1]>1)return false;

        s1.pop_back();
        s.insert(s1);

        if(siz(s)>sz)return false;
...
int main()
{
    ...
    while(cin>>st)
    {
        ...
        if(!vf)puts("not complete");
        else
        {
            printf("%s",v[0].ss.c_str());

            for(int i=1;i<siz(v);i++)
            {
                printf(" %s",v[i].ss.c_str());
                //cout<<x.ff<<"  "<<x.ss<<endl;
            }
            puts("");
